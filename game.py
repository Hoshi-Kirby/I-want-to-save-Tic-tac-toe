...
#のターン
sturn=pygame.image.load("image/'s_turn.png").convert()
...
font = pygame.font.Font("fonts/NotoSansJP-VariableFont_wght.ttf", font_size_max)
...
def menuui():
    ...
    pause_ka=-1
    value.screen.blit(pausename, (width_skew_pause,pausey+math.sin(menut/10)*10))
    for i in range(3):
...
def gameb():
        ...
        for j in range(5):
            ...
            match value.board2[i][j]:
                ...
                case _:
                    pass
    if 10>value.t:
        value.screen.blit(frame, (framex-(10-value.t)*20,framey))
    else:
        value.screen.blit(frame, (framex,framey))

    if value.t<6:
        value.screen.blit(token[value.player-1], (detailx+math.sin(value.t/2*math.pi)*10,sturny))
    else:
    ...
    j=0
    x = cardx+cardx_move/2 * value.spacing/2
    x2 = cardx+cardx_move2/2 * value.spacing*20
    ...
    if value.t<ready_time+ready_time2:
        ...
        value.screen.blit(ready_text, ready_text.get_rect(center=(639.5, 400+(font_size_max-font_size)*2.5)))
    elif value.t<start_time+start_time2+ready_time+ready_time2:
        start_text = font.render("Start!", True, (255, 160, 160))
        value.screen.blit(start_text, start_text.get_rect(center=(639.5, 400+(font_size_max-font_size)*2.5)))
...
def game():
    ...
    if check_win(1):
        ...
        if value.t<finish_time+finish_time2+finisht:
            font = pygame.font.Font("fonts/NotoSansJP-VariableFont_wght.ttf", font_size)
            if value.t<finish_time+finish_time2+finisht:
    ...
    if value.fade_out:
        value.fade_alpha += 40  # フェード速度（調整可）
        if value.fade_alpha >= 255:
            value.fade_alpha = 255
            if value.nextstep==1:
                value.step=5
    ...
    if value.gamestep==1:value.t+=10
    menut+=1
...
def change():
    ...
    x = cardx+cardx_move/2 * value.spacing/2
    x2 = cardx+cardx_move2/2 * value.spacing*20
    x3 = cardx2+cardx_move/2 * value.spacing2/2
    x4 = cardx2+cardx_move2/2 * value.spacing2*20
    ...
    if value.t==1 or ((not first) and value.t==4):
        if value.player==1:
    ...
    if (value.card_dcost[2-value.player]!=0 or card_dcost_mode)and value.t<=8:
        for i in range(10):
            value.card_dy[2-value.player][i]=(4-abs(4-value.t))*50
