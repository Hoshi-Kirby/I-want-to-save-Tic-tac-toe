    ...
    if value.skillstep==0:
        ...
        if card_select_number==max(1,value.cost[skillnum]+1+value.card_dcost[value.player-1]) or card_move_time>=0:
            ...
            for i in range(9, -1, -1):
                if card_select[i]>=1:
                    value.card_dx[value.player-1][i]=(5-card_move_time)*speedx
                    if card_move_time==0:
                        if value.player==1:
                            del value.hands[i]
                        else:
                    ...
                    value.card_dx[value.player-1][i]=(card_x_after[i]-card_x_before[i])/5*(5-card_move_time)
            if card_move_time==0:
                value.skillstep=1
                value.t=0
                value.card_dx=[[0]*10,[0]*10]
        ...
        for i in range(0,5):
            for j in range(0,5):
                if value.board[j][i]==3-value.player or value.board[j][i]==3:
                    ch=1
                    draw_sq(j,i,90+40*math.sin(value.t/4))
...
def skill13():
    ...
    if value.skillstep==2:
        ...
        for i in range(9, -1, -1):
            if i==card_select_base:
                value.card_dx[2-value.player][i]=(5-card_move_time)*speedx
                if card_move_time==0:
                    if value.player==2:
                        del value.hands[i]
                    else:
                ...
                value.card_dx[2-value.player][i]=(card_x_after[i]-card_x_before[i])/5*(5-card_move_time)
        if card_move_time==0:
            value.skillstep=0
            value.gamestep=1
            value.t=0
        ...
        for i in (1,3):
            for j in (1,3):
                if value.board2[j][i]==0 or value.board2[j][i]==3-value.player:
                    ch=1
                    draw_ci(j,i,90+40*math.sin(value.t/4))
...
def skill24():
    ...
    if value.player==1:
        for i in range(len(value.hands)-1):
            value.card_dx[0][i] = cardx_move/6 * value.spacing/2
        value.card_dx[0][len(value.hands)-1] = cardx_move2/6 * value.spacing*20
    else:
        for i in range(len(value.hands2)-1):
            value.card_dx[1][i] = cardx_move/6 * value.spacing2/2
        value.card_dx[1][len(value.hands2)-1] = cardx_move2/6 * value.spacing2*20
...
def skill32():
    if value.skillstep==1:
        ...
        click_x=-1
        direction(32,1)
        alpha_t=50+100*math.sin(value.t/4)
...
def skill45():
    ...
    for i in range(10):
        value.card_dy[value.player-1][i]=(4-abs(4-value.t))*50
    if value.t==8:
        value.skillstep=0
        value.gamestep=1
