    ...
    if a>=2:
        if value.event_t==4:
            ...
            if a==3:
                ...
                value.card_dcost[1]+=1
        for i in range(10):
            value.card_dy[0][i]=(4-abs(4-value.event_t))*50
            value.card_dy[1][i]=(4-abs(4-value.event_t))*50
        if value.event_t==8:
            value.eventnum=-1
    value.event_t+=1
...
def handeath():
    ...
    for i in range(9, -1, -1):
        if i==card_select_base[0]:
            value.card_dx[0][i]=(5-card_move_time)*speedx
            if card_move_time==0:
                del value.hands[i]
        else:
            value.card_dx[0][i]=(card_x_after[0][i]-card_x_before[0][i])/5*(5-card_move_time)
        if i==card_select_base[1]:
            value.card_dx[1][i]=(5-card_move_time)*speedx
            if card_move_time==0:
                del value.hands2[i]
        else:
            value.card_dx[1][i]=(card_x_after[1][i]-card_x_before[1][i])/5*(5-card_move_time)
    if card_move_time==0:
